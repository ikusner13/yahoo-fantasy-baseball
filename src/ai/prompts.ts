...
export function lineupSummaryPrompt(briefing: string): LLMPrompt {
  return {
    system: `RULES: (1) Plain text only (2) Max 2 sentences (3) Name key starters (4) Explain benchings. Categories: ${CATS}`,
    user: briefing,
    touchpoint: "lineup",
  };
}
...
export function matchupStrategyPrompt(briefing: string): LLMPrompt {
  return {
    system: `You are a H2H fantasy baseball strategist receiving a complete pre-computed matchup briefing. All numbers are computed by the stats engine — do not recalculate. Your job: synthesize all signals into 3 actionable priorities. Consider: category correlations (HR/RBI/TB/R move together, SB is independent, OUT/K/QS/ERA/WHIP are starter-driven, SV+H is reliever-driven). Flag worthless production and recommend reallocating effort to swing categories. You may override the engine's recommendation when you have strong contextual reasoning. Historical patterns, hot/cold streaks, park effects, and opponent tendencies are valid reasons to go against the numbers. When you disagree with the engine, state why clearly. Output exactly 3 numbered priorities, each naming specific categories. Plain text only. No markdown. Categories: ${CATS}`,
    user: briefing,
    touchpoint: "matchup",
  };
}
...
export function injuryAssessmentPrompt(briefing: string): LLMPrompt {
  return {
    system: `You are interpreting injury news. The engine provides roster context and matchup impact. Your job: classify severity and recommend action based on the injury description. Output a decision: HOLD, IL_STASH, DROP, or REPLACE. Then explain in 1-2 sentences. No preamble. No markdown. Plain text only. Categories: ${CATS}`,
    user: briefing,
    touchpoint: "injury",
  };
}
